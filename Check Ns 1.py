...
create_table_query = "CREATE TABLE SummaryTable (FieldName TEXT, " + " INTEGER, ".join(map(str, range(10))) + " INTEGER)"
cursor.execute(create_table_query)

# Count occurrences of each value in each column and insert into the SummaryTable
for col in columns_to_analyze:
    count_query = (
        f"INSERT INTO SummaryTable (FieldName, {', '.join(map(str, range(10)))}) "
        f"SELECT '{col}', {', '.join([f'SUM(IIF([{col}]={i}, 1, 0))' for i in range(10)])} FROM Counting"
    )
